...
type NavItem = {
  ...
  active?: boolean;
  soon?: boolean;
  /** Título de sección que se muestra ANTES de este ítem (inicia un grupo). */
  section?: string;
...
const NAV: NavItem[] = [
  ...
  // ── Sección ASISTENTE: el cerebro del asistente (persona + lo que sabe + cómo actúa) ──
  { href: "/agentes", label: "Asistente", section: "Asistente", indent: true, icon: <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>, active: true },
  { href: "/conocimiento", label: "Documentos", indent: true, icon: <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>, active: true },
  { href: "/memoria", label: "Datos del negocio", indent: true, icon: <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" /></svg>, active: true },
  { href: "/automatizaciones", label: "Auto-respuestas", indent: true, icon: <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>, active: true },

  // ── Sección CLIENTES ──
  { href: "/crm", label: "CRM", section: "Clientes", icon: <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z" /></svg>, active: true },
  ...
  { href: "/estado", label: "Estado del sistema", adminOnly: true, icon: <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>, active: true },
];
...
export default async function EmpresaLayout({
  ...
  const seccionesRestringidas = ["Documentos", "Datos del negocio", "Auto-respuestas", "Configuración"];
        ...
        <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
          {navFiltrada.map((item) => {
            ...
            return (
              <div key={item.label}>
                {item.section && (
                  <p className="px-3 pt-4 pb-1 text-[10px] font-semibold uppercase" style={{ color: "#8FA69F", letterSpacing: "0.09em" }}>
                    {item.section}
                  </p>
                )}
                {item.soon ? (
                  <div className={`flex items-center gap-3 ${padding} py-2 rounded-lg text-sm opacity-30 cursor-not-allowed select-none`} style={{ color: "#A9BED2" }}>
                    {item.icon}
                  ...
                  <ActiveNavLink
                    href={href}
                    exact={item.href === ""}
                    className={`flex items-center gap-3 ${padding} py-2 rounded-lg text-sm transition-all text-gray-600 hover:text-gray-900 hover:bg-gradient-to-r hover:from-blue-50 hover:to-emerald-50 group`}
